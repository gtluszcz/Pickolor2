...
class ColorsController extends Controller
{
    ...
    public function showall(Request $request)
    {
        switch ($request->query('order','likes')){
            case 'likes':
                $color = App\Colors::orderBy('likes', 'desc')->paginate(30);
                break;
            case 'oldest':
                $color = App\Colors::orderBy('updated_at', 'desc')->paginate(30);
                break;
            case 'newest':
                $color = App\Colors::orderBy('updated_at', 'asc')->paginate(30);
                break;
    ...
    public function showmyfavourite(Request $request)
    {
        ...
        switch ($request->query('order','likes')){
            case 'likes':
                ...
                break;
            case 'oldest':
                $colors = $user->fav_colors()->orderBy('updated_at', 'desc')->paginate(30);
                break;
            case 'newest':
                $colors = $user->fav_colors()->orderBy('updated_at', 'asc')->paginate(30);
                break;
